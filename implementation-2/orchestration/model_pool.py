# IMP2 Part 2 -- shared model loading for the WebShop 2-role orchestration.
#
...
#   - "shared":       used by both search_specialist and compare_specialist
#                      (the "share weights/adapter with search-specialist as
#                      the starting default" spec from IMP2 Part 2).
...
def get_shared_model(device: str = "cuda:0"):
    ...
    `model` carries both the "shared" and "orchestrator" adapters; callers
    must call `use_adapter(model, name)` before generating/training to select
    which one is active."""
...
@torch.no_grad()
def generate(model, tokenizer, system_prompt: str, user_prompt: str, cfg: Optional[GenerationConfig] = None) -> str:
