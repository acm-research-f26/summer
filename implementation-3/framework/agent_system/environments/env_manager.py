...
class AlfWorldEnvironmentManagerNothink(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        super().__init__(envs, projection_f, config)
    ...
    def build_text_obs(self, text_obs: List[str], admissible_actions: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            if init or self.config.env.history_length <= 0:
                obs = ALFWORLD_TEMPLATE_NOTHINK_NO_HIS.format(
                    current_observation=text_obs[i],
                    admissible_actions=reformatted_admissible_actions
                )
            else:
                obs = ALFWORLD_TEMPLATE_NOTHINK.format(
                    task_description=self.tasks[i],
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
                    action_history=memory_contexts[i],
...
class WebshopEnvironmentManagerOptions(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        super().__init__(envs, projection_f, config)
    ...
    def build_text_obs(self, text_obs: List[str], infos: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            reformatted_available_actions = "\n".join(f"'{s}'," for s in available_actions)
            if init or self.config.env.history_length <= 0:
                obs = WEBSHOP_TEMPLATE_OPTIONS_NO_HIS.format(
                    task_description=self.tasks[i],
                    current_observation=text_obs[i],
                    available_actions=reformatted_available_actions,
                    current_subgoal="None"
                ...
                obs = WEBSHOP_TEMPLATE_OPTIONS.format(
                    task_description=self.tasks[i],
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
                    action_history=memory_contexts[i],
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        if config.env.env_name == 'Webshop':
            ...
            return envs, val_envs
        elif config.env.env_name == 'WebshopOptions':
            from agent_system.environments.env_package.webshop import webshop_projection_options
            _envs = build_webshop_envs(seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, is_train=True, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
            _val_envs = build_webshop_envs(seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, is_train=False, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)

            projection_f = partial(webshop_projection_options)
            envs = WebshopEnvironmentManagerOptions(_envs, projection_f, config)
            val_envs = WebshopEnvironmentManagerOptions(_val_envs, projection_f, config)
            import time
            time.sleep((config.data.train_batch_size * group_n + config.data.val_batch_size) * 0.1) # wait for the envs to be ready
            return envs, val_envs
