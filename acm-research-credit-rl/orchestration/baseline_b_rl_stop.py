# IMP2 Part 4 -- Baseline B: RL-trained stop policy, outcome-only reward.
#
# R_episode = 10 * success - c * num_delegation_rounds, c=0.5 starting point
# (not tuned here, that's Implementation 3). Trains the orchestrator's
# {delegate-search, delegate-compare, stop} decision via GRPO: sample a group
# of G orchestrator rollouts per task, compute cost-adjusted return per
...
class BaselineBTrainer:
    def __init__(
        ...
        self.search = SearchSpecialist()
        self.compare = CompareSpecialist()
    ...
    def collect_group(self, env) -> List[RolloutRecord]:
        ...
        for _ in range(self.group_size):
            ...
            log = run_episode(env, self.orchestrator, self.search, self.compare, round_cap=self.round_cap)
...
def train_baseline_b(num_groups: int = 4, group_size: int = 4, cost_coef: float = DEFAULT_COST_COEF) -> List[dict]:
    env = make_env()
