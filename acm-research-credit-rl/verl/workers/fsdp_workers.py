...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        if self.config.use_three_heads_critic:
            critic_model_config.num_labels = 3
        elif self.config.use_two_heads_critic:
    ...
    def compute_values(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.config.use_three_heads_critic:
                output = DataProto.from_dict(tensors={"value_low": values[0], "value_high": values[1], "value_term": values[2]})
            elif self.config.use_two_heads_critic:
                output = DataProto.from_dict(tensors={"value_low": values[0], "value_high": values[1]})
            else:
