...
# Layer Importance Ranking Benchmark
...
We propose a **2×2 taxonomy of data-free sensitivity metrics** along two axes — **information source** and **propagation awareness** — and show empirically that the optimal metric is **architecture-dependent**: numerical metrics (PCT) dominate on CNNs while spectral metrics (SEA, Spectral Top-10) dominate on vision transformers. See [`../README.md`](../README.md) for the full taxonomy including LLM results from Part 2.

### Metric taxonomy (Part 1 scorers)

|  | **Uses weight values** | **Spectral only** |
|--|------------------------|-------------------|
| **No propagation** | **PCT**, **Entropy** | **SEA** |
| **With propagation** | *(Spectral-PCT — Part 2)* | — |

- **PCT (Per-Channel Truncation)** *(ours)*: Numerical, no propagation — scores each layer by relative Frobenius error after **per-channel** symmetric quantization. Asks: *which layers lose the most information when actually quantized?*
- **Entropy** *(ours)*: Numerical, no propagation — scores each layer by **Shannon entropy of its weight histogram**; captures dispersed, heterogeneous weights that are harder to compress in few bits.
- **SEA (Spectral Energy Asymmetry)**: Spectral only, no propagation — inverse SVD-entropy of the weight tensor; captures spectral structure without reading raw magnitudes.
- **Architecture-aware guidance**: CNNs → weight-values quadrant (**PCT**); ViTs → spectral or distributional quadrant (**Entropy**, **SEA**).
...
*NSDS is **prior work** included for comparison.*
...
**Next step:** See [`../README.md`](../README.md) for Part 2 — turning these rankings into **proportional bit-budget allocations** under target average bit-widths.
