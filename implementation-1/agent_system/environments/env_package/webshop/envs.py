...
class WebshopWorker:
    ...
    def __init__(self, seed, env_kwargs):
        ...
        self.early_terminate_penalty = float(env_kwargs.get("early_terminate_penalty", 0.0))
    ...
    def step(self, action):
        ...
        # Redefine reward. We only use rule-based reward - win for 10, lose for 0 (or penalty).
        if done and reward == 1.0:
            info['won'] = True
            reward = 10.0
        else:
            info['won'] = False
            if done and self.early_terminate_penalty != 0.0:
                reward = float(self.early_terminate_penalty)
            else:
                reward = 0
